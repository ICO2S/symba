...
public class CisbanProtocolHelper implements MappingHelper<Protocol, FugeOMCommonProtocolProtocolType> {
    ...
    public CisbanProtocolHelper() {
        this.ci = new CisbanIdentifiableHelper();
        this.cparam = new CisbanParameterizableHelper();
        this.cgp = new CisbanGenericProtocolHelper();
        this.helper = CisbanHelper.create( reService );
    }

    // fixme: Assumes XML is Master version: that EVERY object that can be loaded (i.e. isn't a reference only) is newer than the database
    public Protocol unmarshal( FugeOMCommonProtocolProtocolType protocolXML, Protocol protocol )
        ...
        if ( protocolXML instanceof FugeOMCommonProtocolGenericProtocolType ) {
            ...
            genericProtocol = ( GenericProtocol ) ci.unmarshal( protocolXML, genericProtocol );
            genericProtocol = ( GenericProtocol ) cparam.unmarshal( protocolXML, genericProtocol );
    ...
    public FugeOMCommonProtocolProtocolType marshal( FugeOMCommonProtocolProtocolType protocolXML, Protocol protocol )
            throws RealizableEntityServiceException {
    ...
    public FugeOMCommonProtocolProtocolType generateRandomXML( FugeOMCommonProtocolProtocolType protocolXML ) {
        // get protocol attributes
        protocolXML = ( FugeOMCommonProtocolProtocolType ) ci.generateRandomXML( protocolXML );
    ...
    public FugeOMCommonProtocolProtocolType generateRandomXML( FugeOMCommonProtocolProtocolType protocolXML,
        ...
        FugeOMCommonProtocolGenericProtocolType genericProtocolXML = ( FugeOMCommonProtocolGenericProtocolType ) generateRandomXML( protocolXML );
    ...
    public Protocol getLatestVersion( Protocol protocol ) throws RealizableEntityServiceException {
        ...
        if ( protocol instanceof GenericProtocol ) {
            // get protocol attributes
            protocol = ( Protocol ) cparam.getLatestVersion( protocol );
    ...
    public void prettyHtml( GenericAction parentGA,
        ...
        if ( size > 0 ) {
            ...
            CisbanProtocolApplicationHelper capp = new CisbanProtocolApplicationHelper();
