...
public class CisbanIdentifiableHelper implements MappingHelper<Identifiable, FugeOMCommonIdentifiableType> {
    private final int NUMBER_ELEMENTS = 2;
    private final CisbanDescribableHelper cd;

    public CisbanIdentifiableHelper() {
    ...
    public Identifiable unmarshal( FugeOMCommonIdentifiableType identifiableXML, Identifiable identifiable )
            throws RealizableEntityServiceException {

        identifiable = ( Identifiable ) cd.unmarshal( identifiableXML, identifiable );
        ...
        for ( FugeOMCommonReferencesDatabaseEntryType databaseEntryXML : identifiableXML.getDatabaseEntry() ) {
            ...
            databaseEntry = ( DatabaseEntry ) cd.unmarshal( databaseEntryXML, databaseEntry );
    ...
    public FugeOMCommonIdentifiableType marshal( FugeOMCommonIdentifiableType identifiableXML,
                                                 Identifiable identifiable ) {

        identifiableXML = ( FugeOMCommonIdentifiableType ) cd.marshal( identifiableXML, identifiable );
        ...
        for ( Object databaseEntryObj : identifiable.getDatabaseReferences() ) {
            ...
            databaseEntryXML = ( FugeOMCommonReferencesDatabaseEntryType ) cd
                    .marshal( databaseEntryXML, databaseEntry );
    ...
    public FugeOMCommonIdentifiableType generateRandomXML( FugeOMCommonIdentifiableType type ) {
        CisbanReferenceableCollectionHelper crc = new CisbanReferenceableCollectionHelper();
        type = ( FugeOMCommonIdentifiableType ) cd.generateRandomXML( type );
        ...
        if ( type instanceof FugeOMCollectionFuGEType ) {
            ...
            for ( int i = 0; i < NUMBER_ELEMENTS; i++ ) {
                FugeOMCommonReferencesDatabaseEntryType databaseEntryXML = new FugeOMCommonReferencesDatabaseEntryType();
                databaseEntryXML = ( FugeOMCommonReferencesDatabaseEntryType ) cd
                        .generateRandomXML( databaseEntryXML );
    ...
    public Identifiable getLatestVersion( Identifiable current ) throws RealizableEntityServiceException {
        ...
        current.setDatabaseReferences( set0 );

        // prepare updated set
        Set<BibliographicReference> set = new HashSet<BibliographicReference>();

        // load all the latest versions into the new set.
        for ( Object obj : current.getBibliographicReferences() ) {
        ...
        current.setBibliographicReferences( set );
