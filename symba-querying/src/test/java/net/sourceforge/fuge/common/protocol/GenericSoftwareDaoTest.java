...
public class GenericSoftwareDaoTest {
    ...
    public void getByNameAndVersionSimpleTest() {
        ...
        assert ( retrievedOne != null ) :
                "The GenericSoftware " + one.getName() + " has not been retrieved by the query";

        assert ( retrievedOne.getName().equals( one.getName() ) ) :
                "The GenericSoftware " + one.getName() + " name does not match the retrieved" +
                "name of " + retrievedOne.getName();

        assert ( retrievedOne.getName().equals( one.getName() ) ) :
                "The GenericSoftware " + one.getVersion() + " version does not match the retrieved" +
                "version of " + retrievedOne.getVersion();

        assert ( retrievedThree != null ) :
                "The GenericSoftware " + three.getName() + " has not been retrieved by the query";

        assert ( retrievedThree.getName().equals( three.getName() ) ) :
                "The GenericSoftware " + three.getName() + " name does not match the retrieved" +
                "name of " + retrievedThree.getName();

        assert ( retrievedThree.getName().equals( three.getName() ) ) :
                "The GenericSoftware " + three.getVersion() + " version does not match the retrieved" +
                "version of " + retrievedThree.getVersion();
    ...
    public void getByNameAndVersionComplexTest() {
        ...
        GenericSoftware two =
                writeGenericSoftware( "getByNameAndVersionComplexTest2", gc.getTime(), one.getEndurant() );

        GenericSoftware retrievedOne = ses.getGenericSoftwareByNameAndVersion(one.getName(), one.getVersion() );
        GenericSoftware retrievedTwo = ses.getGenericSoftwareByNameAndVersion( two.getName(), two.getVersion() );

        assert ( retrievedOne == null ) :
                "The GenericSoftware " + one.getName() + " should not be retrieved by the query as it is an " +
                "older version";

        assert ( retrievedTwo != null ) :
                "The GenericSoftware " + two.getName() + " has not been retrieved by the query";

        assert ( retrievedTwo.getName().equals( two.getName() ) ) :
                "The GenericSoftware " + two.getName() + " name does not match the retrieved" +
                "name of " + retrievedTwo.getName();

        assert ( retrievedTwo.getName().equals( two.getName() ) ) :
                "The GenericSoftware " + two.getVersion() + " version does not match the retrieved" +
                "version of " + retrievedTwo.getVersion();
    }
    ...
    private GenericSoftware writeGenericSoftware( String name,
                                                        Date time,
                                                        Endurant endurant ) {

        EntityService es = ServiceLocator.instance().getEntityService();

        GenericSoftware genericSoftware = ( GenericSoftware ) es
                .createIdentifiable( name + ":GenericSoftware:" + String.valueOf( Math.random() * 10000 ), name,
        ...
        // GenericSoftwares must have a term
        ...
        genericSoftware =
                ( GenericSoftware ) es
                        .create( "net.sourceforge.fuge.common.protocol.GenericSoftware", genericSoftware );
