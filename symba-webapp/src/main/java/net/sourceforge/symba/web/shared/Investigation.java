...
//        new Investigation( "00", "", new Contact("0", "", "", "") );
    ...
    public Investigation( Investigation investigation ) {
        this( investigation.isTemplate(),
              investigation.isCompleted(),
              investigation.getId(),
              investigation.getInvestigationTitle(),
              investigation.getHypothesis(),
              investigation.getConclusion(),
     ...
     * @return the new title at [0] and presence of modification (true/false) at [1]. Will be empty array if no matches
     *         found
     */
...
//        System.err.println( "Adding at level " + selectedRow );
    ...
    /**
     * Performs a deep copy of the experiment represented with the given selected row. As the null value for the step is
     * used to show whether or not the step has already been added, it cannot definitely also be used to determine if
     * anything has been added to the investigation at all. Therefore it makes sense to only return void.
     *
