...
public interface LsidAssigner {
    ...
    /**
     * It returns a full LSID for a data entity that has the authority and namespace as requested by caller.
     * Service returns null if it cannot or does not want to name the object.
     * <p/>
     * This fails the spec in that the argument is a String rather than an LSID object, but none of the
     * other parts of the LSID object are used within SyMBA. It also doesn't use the property elements mentioned
     * in the spec.
     *
    ...
    /**
     * Same as assignLsid but uses a default prefix that can be set by the SyMBA developer.
     *
     * @param namespace the namespace of the identifier. In SyMBA, it is expected that this is the class name
     * @return null if it cannot or does not want to name the object, or the lsid string otherwise
     */
