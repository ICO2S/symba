...
import net.sourceforge.fuge.util.generated.FuGECommonProtocolProtocolType;
...
public class ServerDatabaseController extends FugeDatabaseController {
    ...
    public HashMap<String, Investigation> convertFugeToGwt() {
        ...
        List<FuGECollectionFuGEType> fugeList = symbaDao.fetchAllFuge();
        for ( FuGECollectionFuGEType fuge : fugeList ) {
...
//                    initProvider( fuge ), initExperiments( fuge.getProtocolCollection() ) );
    ...
    @NotNull
    private Contact initProvider( @NotNull FuGECollectionFuGEType fuge ) {
        for ( FuGECollectionAuditCollectionType.FuGECollectionAuditCollectionTypeContactItem item : fuge
                .getAuditCollection().getContactItems() ) {
            if ( item.getItemValue() instanceof FuGECommonAuditPersonType ) {
                FuGECommonAuditPersonType contact = ( FuGECommonAuditPersonType ) item.getItemValue();
    ...
    private void buildExperimentHierarchy( ArrayList<ExperimentStepHolder> stepList,
                                           List<FuGECollectionProtocolCollectionType.FuGECollectionProtocolCollectionTypeProtocolItem> pItems ) {
        for ( FuGECollectionProtocolCollectionType.FuGECollectionProtocolCollectionTypeProtocolItem item : pItems ) {
            FuGECommonProtocolProtocolType protocol = item.getItemValue();
