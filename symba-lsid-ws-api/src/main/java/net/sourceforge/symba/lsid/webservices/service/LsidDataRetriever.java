 ...
 * however the getDataByRange() and the getMetadataSubset() are yet to be implemented. The spec says:
 * <p/>
 ...
 * <p/>
 * The data retrieval services may implement all of the methods, or only methods for retrieving data, or only methods for
 * retrieving associated metadata.
 * <p/>
 * The same LSID named data object must be resolved always to the same set of bytes. Therefore, all of the data retrieval
 * services return the same results for the same LSID. The user has, however, the choice of which one of these to utilize
 * depending on its location, known quality of service and other attributes.
...
public interface LsidDataRetriever {
     ...
     * From the lsid spec:
     * <p/>
     * "This method is used to return document containing the metadata associated with a particular lsid at this particular data
     * retrieval service. Note that this means that calling getMetadata on two different data services may yield different metadata
     * since each service may contain different metadata about the same lsid.
     * <p/>
     ...
     * @return null if not passed an acceptable format, empty class if no information, otherwise the metadata from
     *         the given lsid
     */
    ...
    /**
     * From the lsid spec:
     * "This method is used to return data associated with the given lsid. If a copy of the data represented by an LSID
     * cannot be returned for any reason, an exception should be raised.
     * <p/>
     * If the given lsid represents an abstract entity (a concept), this method returns an empty array of bytes.
     * Note that the semantics of the returned bytes is not defined by this specification. It is either known from an external
     * documentation, or (preferably) it is available by reading the metadata for this particular lsid."
     *
